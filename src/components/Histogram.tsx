...
function Histogram({}: Props) {
    ...
    useEffect(() => {
        ...
        img.addEventListener("load", () => {
            ...
            const chartCtx = (
                canvasChartRef.current! as HTMLCanvasElement
            ).getContext("2d");
    ...
    const equalizeExpansion = () => {
        ...
        let equalizedPixels = pixelsDataGrayscale.map((pixel) => {
            return Math.round(
                ((pixel[0] - minGrayscale) / (maxGrayscale - minGrayscale)) *
                    256
            );
        });
    ...
    const equalize = () => {
        ...
        let lookUpTable = (k: number) =>
            Math.round(((sk(k) - sk(0)) / (1 - sk(0))) * 256);
        ...
        for (let pixel of pixelsFlat) {
            chartData[lookUpTable(pixel)] =
                (chartData[lookUpTable(pixel)] ?? 0) + 1;
        }
